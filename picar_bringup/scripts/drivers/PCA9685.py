...
class PWM(object):
  ...
  def _write_byte_data(self, reg, value):
    '''Write data to I2C with self.__address'''
  ...
  def _read_byte_data(self, reg):
    '''Read data from I2C with self.__address'''
    self._debug_('Reading value from %2X' % reg)
    try:
      results = self.__bus.read_byte_data(self.__address, reg)
      return results
    except Exception as i:
      print(i)
      self._check_i2c()
  ...
  def _check_i2c(self):
    ...
    print("I2C bus number is: %s" % self.__bus_number)
    print("Checking I2C device:")
    devices = listdir("/dev/")
    if "i2c-%d" % self.__bus_number in devices:
    ...
    if addresses == []:
      ...
      for address in addresses:
        print("  0x%s" % address)
    if "%02X" % self.__address in addresses:
